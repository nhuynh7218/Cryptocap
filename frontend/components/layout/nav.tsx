...
const NavLink = ({ children, props }: { children: ReactNode, props: { currentMenu: string, index: number, link: string, colorMode: string } }) => (

  <NextLink href={`${props.link == 'News' ? '/' : `/${props.link.toLocaleLowerCase()}`}`}>
    <a
      className={`${props.currentMenu == props.link.toLocaleLowerCase() ? 'border-2 border-green-500' : 'border-black'} ${props.colorMode == 'light' ? 'hover:bg-gray-200' : 'hover:bg-gray-700'}  transition-all px-2 py-1 rounded-md`}
    >
      {children}
    </a>
  </NextLink>
...
export default function NavBar() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (items) {
        setUserInfo(items);
        console.log("grabbing ingo", local)
      }
      console.log("sdf")
    }
  ...
  return (
    <>
      ...
      {showAuthModal && <AuthenticateModal isActived={showAuthModal} setUserInfo={setUserInfo} toggleActive={toggleShowWalletConnect} />
      }
          ...
          <HStack spacing={8} alignItems={'center'}>
            ...
            <HStack
              ...
              {Links.map((link, index) => (
                <NavLink props={{ currentMenu, index, link, colorMode }} key={link}>{link}</NavLink>
              ))}
            </HStack>
          </HStack>
...
function AuthenticateModal(props: { isActived: boolean, toggleActive: (state: boolean) => void, setUserInfo: (u: StoredUserInfo) => void }) {
  ...
  */
  ...
  const [authType, setAuthType] = useState(AUTH_TYPE.SIGNUP)
  ...
  function SignUp(props : { setUserInfo: (u: StoredUserInfo) => void, toggleActive: (b: boolean) => void}) {
    ...
    async function SignUp(event: any) {
      ...
      setAppState({appState: APP_STATE.LOADING, title: 'Siging You Up...', msg: ''})
    ...
    return (
      <form onSubmit={(e) => SignUp(e)} className='flex flex-col space-y-2 pt-2 text-black px-8 '>
        ...
        <input required value={confirmPassword} onChange={(input) => setConfirmPassword(input.target.value)} type="password" className={`${confirmPassword == '' ? 'border-gray-500' : password == confirmPassword ? 'border-green-600' : 'border-red-500'} border-4 px-1 p-1 rounded`} placeholder='Confirm Passowrd...'></input>
              ...
              {authType == AUTH_TYPE.SIGNUP &&
                <AnimatePresence>
                  <motion.div initial={{ y: "-50%", opacity: 0 }} animate={{ y: "0%", opacity: 1 }} exit={{ y: "50%", opacity: 0 }}>
                    ...
                    <SignUp setUserInfo={props.setUserInfo} toggleActive={props.toggleActive} />
                  </motion.div>
                </AnimatePresence>
              }
              {authType == AUTH_TYPE.LOGIN &&
                <AnimatePresence>
                  <motion.div initial={{ y: "-50%", opacity: 0 }} animate={{ y: "0%", opacity: 1 }} exit={{ y: "50%", opacity: 0 }}>
                    ...
                    <LogIn setUserInfo={props.setUserInfo} toggleActive={props.toggleActive} />
                  </motion.div>
                </AnimatePresence>
              }
            </div>
  ...
  return (
    <>
      ...
      {props.isActived && 
        ...
        <div >
          <div className="min-w-screen h-screen fixed  left-0 top-0 flex justify-center items-center inset-0 z-40 outline-none focus:outline-none  ">
            <div ref={ref}>
              {props.children}
            </div>
          </div>
